...
pstat_test_1_f1 <- test_method_1(bind_rows(x_ic, x_f1), mu_0, sigma_0, lambda_s = .2)
pstat_test_1_f2 <- test_method_1(bind_rows(x_ic, x_f2), mu_0, sigma_0, lambda_s = .2)
pstat_test_1_f3 <- test_method_1(bind_rows(x_ic, x_f3), mu_0, sigma_0, lambda_s = .2)
pstat_test_1_f4 <- test_method_1(bind_rows(x_ic, x_f4), mu_0, sigma_0, lambda_s = .2)
...
h_test_1 <- estimate_h(pstat_test_1_f1$pstat[1:1000], 200)
h_hawkins_comet <- estimate_h(pstat_hawkins_comet_f1$pstat[1:1000], 200)



# Calculated IC vs OC Observations ----------------------------------------
...
df_test_1 <-
  bind_rows(list(f1 = pstat_test_1_f1, 
                 f2 = pstat_test_1_f2, 
                 f3 = pstat_test_1_f3,
                 f4 = pstat_test_1_f4), .id = "fault") |> 
  mutate(fault = as_factor(fault),
         index = 1:n(), .by = "fault")
...
df_test_1 |> 
  ...
  geom_point(shape = 21) +
  geom_vline(xintercept = 1000, color = "red", linetype = "dashed") +
  geom_hline(yintercept = h_test_1, color = "darkgreen", linetype = "dashed") +
  facet_wrap(~ fault, ncol = 1, scales = "free_x", labeller = fault_labels) +
  labs(y = "Plotting Statistic",
       title = "Test Chart 1: Sample Plot of c_t with sparsified S")

ggsave(here("figures", "test_1_pstat.png"), width = width, height = height, units = 'px')
...
df_test_1 |> 
  ...
  labs(y = "Components of Hawkins Sparse Plotting Statistic",
       title = "Test Chart 1: Plotting Statistic Breakdown",
       color = "")

ggsave(here("figures", "test_1_breakdown.png"), width = width, height = height, units = 'px')
