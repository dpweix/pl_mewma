...
generate_ggheat_plots <- function(df, method = "Sample", data_lab = "") {
  map(1:length(uf_states_reduced), \(i) {
    df |> 
      ...
      labs(title = glue("{data_lab} {str_to_title(method)} Correlation: {uf_states_reduced_nice_names[[i]]}"), x = "", y = "", fill = "")
  })
}
...
gg_pstat <- function(df, method = "hawkins", data = "Raw") {
  if(method %in% c("wang_2", "MAC_COMET")) {
    p1 <- 
      df[[method]] |> 
      ggplot(aes(Date_Time, T_1, color = `UF State`, group = 1)) +
      geom_line() +
      scale_color_viridis_d() +
      scale_x_datetime(limits = time_exp_3_limits,
                       date_breaks = "12 hours",
                       date_labels = "%m/%d%n%H:%M") +
      geom_line(aes(y = h1), color = "black") + 
      geom_vline(xintercept = begin_fault, color = "blue", linetype = "dashed") +
      geom_vline(xintercept = end_fault, color = "blue", linetype = "dashed") +
      labs(x = "", y = "", title = glue("{get_name(method)} Charting Statistic: T_1"))
    
    p2 <- 
      df[[method]] |> 
      ggplot(aes(Date_Time, T_2, color = `UF State`, group = 1)) +
      geom_line() +
      scale_color_viridis_d() +
      scale_x_datetime(limits = time_exp_3_limits,
                       date_breaks = "12 hours",
                       date_labels = "%m/%d%n%H:%M") +
      geom_line(aes(y = h2), color = "black") +
      geom_vline(xintercept = begin_fault, color = "blue", linetype = "dashed") +
      geom_vline(xintercept = end_fault, color = "blue", linetype = "dashed") +
      labs(x = "", y = "", title = glue("{get_name(method)} Charting Statistic: T_2"))
    
    list(p1, p2)
  } else {
    df[[method]] |> 
      ggplot(aes(Date_Time, pstat, color = `UF State`, group = 1)) +
      geom_line() +
      scale_color_viridis_d() +
      scale_x_datetime(limits = time_exp_3_limits,
                       date_breaks = "12 hours",
      ...
      geom_vline(xintercept = begin_fault, color = "blue", linetype = "dashed") +
      geom_vline(xintercept = end_fault, color = "blue", linetype = "dashed") +
      labs(x = "", y = "", title = glue("{get_name(method)} Charting Statistic"))
  }
}
